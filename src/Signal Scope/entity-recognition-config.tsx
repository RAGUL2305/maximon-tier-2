    ...
    <div className="bg-white rounded-lg shadow p-6 max-w-4xl mx-auto">
      ...
      <div className="bg-gray-50 p-4 rounded-md mb-6">
        <p className="text-sm text-gray-600">
          Entity Recognition maps external mentions to your brand taxonomy.
          Configure the entities you want to track across channels, social
          media, and LLM outputs.
        </p>
      </div>
            ...
            <div>
              ...
              <div className="flex items-center">
                ...
                <span className="ml-2 text-gray-700">
                  {(newEntity.confidence * 100).toFixed(0)}%
                </span>
              </div>
            </div>
          </div>
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {entities.map((entity) => (
              <tr
                key={entity.id}
                className={!entity.active ? "bg-gray-50" : ""}
              >
      ...
      {testResults && selectedEntity && (
        <div className="mt-6 border rounded-md p-4">
          <h3 className="font-medium text-gray-800 mb-2 flex items-center">
            Test Results:{" "}
            <span className="text-indigo-600 ml-1">{selectedEntity.name}</span>
          </h3>
            ...
            <ul className="space-y-2 text-sm">
              {testResults.sampleTexts.map((text: any, index: any) => (
                <li key={index} className="p-2 bg-white rounded border">
                  {text.replace(
                    "[ENTITY]",
                    `<span class="font-semibold text-indigo-600">${selectedEntity.name}</span>`
                  )}
                </li>
              ))}
            </ul>
