...
const JourneyValidator = () => {
  ...
  interface ValidationResult {
    brandCompliance: {
      status: string;
      issues: { severity: string; message: string; nodeId?: number }[];
      passRate: number;
    };
    driftCheck: {
      status: string;
    ...
    triggerHealth: {
      status: string;
      issues: { severity: string; message: string; triggerId?: string }[];
      passRate: number;
    };
  }
      ...
      setValidationResult({
        brandCompliance: {
          ...
          issues: [
            {
              severity: "warning",
              message: "CTA tone in Node 4 deviates from brand guidelines",
              nodeId: 4,
            },
            {
              severity: "warning",
              message:
                "Product description in Node 7 uses non-approved terminology",
              nodeId: 7,
            },
          ],
          passRate: 83,
        ...
        triggerHealth: {
          ...
          issues: [
            {
              severity: "error",
              message: 'Trigger "abandoned_cart" missing fallback path',
              triggerId: "trigger-002",
            },
            {
              severity: "warning",
              message:
                'Trigger "email_open" has potential overlap with "email_click"',
    ...
    <div className="bg-white rounded-lg shadow overflow-hidden">
      ...
      <div className="p-4 border-b">
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          <div>
            ...
            <p className="font-medium">{journeyData?.name || "N/A"}</p>
          </div>
          <div>
          ...
          <div>
            ...
            <p className="font-medium">
              {journeyData
                ? new Date(journeyData.lastModified).toLocaleString()
                : "N/A"}
            </p>
          </div>
        </div>
      ...
      <div className="divide-y">
            ...
            <div className="flex items-center space-x-3">
              ...
              <span
                className={`px-2 py-0.5 text-xs rounded-full ${
                  (validationResult?.brandCompliance.passRate ?? 0) > 90
                    ? "bg-green-100 text-green-800"
                    : (validationResult?.brandCompliance.passRate ?? 0) > 70
                    ? "bg-yellow-100 text-yellow-800"
                    : "bg-red-100 text-red-800"
                }`}
              >
                {validationResult?.brandCompliance?.passRate ?? 0}% Pass
              </span>
            </div>
            ...
            <div className="mt-3 pl-8">
              ...
              validationResult.brandCompliance.issues.length === 0 ? (
                ...
                <ul className="space-y-2">
                  {validationResult?.brandCompliance.issues.map(
                    (issue, idx) => (
              ...
              <div className="mt-4 p-3 bg-gray-50 rounded-md">
                <p className="text-sm text-gray-600">
                  <strong>Brand alignment check:</strong> Validates that all
                  content, CTAs, and messaging align with brand voice guidelines
                  stored in Memory Loom.
                </p>
              </div>
            </div>
          )}
        ...
        <div className="p-4">
          ...
          >
            <div className="flex items-center space-x-3">
              ...
              <span
                className={`px-2 py-0.5 text-xs rounded-full ${
                  (validationResult?.driftCheck?.passRate ?? 0) > 90
                    ? "bg-green-100 text-green-800"
                    : validationResult?.driftCheck?.passRate ?? 0 > 70
                    ? "bg-yellow-100 text-yellow-800"
                    : "bg-red-100 text-red-800"
                }`}
              >
                {validationResult?.driftCheck?.passRate ?? 0}% Pass
              </span>
            </div>
          ...
          {expandedSections.drift && (
            <div className="mt-3 pl-8">
              {validationResult && validationResult.driftCheck.issues.length === 0 ? (
                <p className="text-green-600 text-sm">
              ...
              <div className="mt-4 p-3 bg-gray-50 rounded-md">
                <p className="text-sm text-gray-600">
                  <strong>Drift detection:</strong> Identifies when journey
                  paths have deviated from expected behavior or brand standards
                  over time.
                </p>
              </div>
            </div>
        ...
        <div className="p-4">
          ...
          >
            <div className="flex items-center space-x-3">
              {validationResult && getStatusIcon(validationResult.triggerHealth.status)}
              <h3 className="font-medium">Trigger Health</h3>
              <span
                className={`px-2 py-0.5 text-xs rounded-full ${
                  (validationResult?.triggerHealth?.passRate ?? 0) > 90
                    ? "bg-green-100 text-green-800"
                    :(validationResult?.triggerHealth.passRate ?? 0) > 70
                    ? "bg-yellow-100 text-yellow-800"
                    : "bg-red-100 text-red-800"
                }`}
              >
                {validationResult?.triggerHealth?.passRate ?? 0}% Pass
              </span>
            </div>
          ...
          {expandedSections.trigger && (
            <div className="mt-3 pl-8">
              {validationResult?.triggerHealth.issues.length === 0 ? (
                <p className="text-green-600 text-sm">
                  No trigger health issues detected.
                </p>
              ) : (
                <ul className="space-y-2">
              ...
              <div className="mt-4 p-3 bg-gray-50 rounded-md">
                <p className="text-sm text-gray-600">
                  <strong>Trigger validation:</strong> Ensures all triggers are
                  properly configured with appropriate fallback paths and don't
